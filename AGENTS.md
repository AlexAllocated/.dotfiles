...
- The `macos-docker` profile also installs a user `launchd` host daemon at `~/.local/share/dotfiles/hostd/hostd.sock`; the container mounts its runtime directory at `/run/host-services/dotfiles-hostd`, and `dotctl host op ...` runs the host `op` binary through an allowlisted JSON socket API.
