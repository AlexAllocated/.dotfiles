...
if ($EnsureCableCapacity) {
   ...
   if (-not (Test-Path -LiteralPath $softwareKey)) {
      throw "Install Virtual Audio Cable before provisioning AMPS cables."
   }
   ...
   if ($oldSoftwareCount -ge 7 -and $oldDriverCount -ge 7 -and (Test-VacCapacity)) {
      ...
      Write-Host "VAC already exposes all seven AMPS cables; no elevation or restart needed."
      exit 0
   }
   $principal = [Security.Principal.WindowsPrincipal]::new([Security.Principal.WindowsIdentity]::GetCurrent())
   if (-not $principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
      if (-not $LogPath) { $LogPath = Join-Path $env:TEMP "amps-vac-capacity.log" }
   ...
   $binaryRoot = Join-Path $env:LOCALAPPDATA "AMPS\bin"
   $processes = @(Get-Process amps,amps-ui -ErrorAction SilentlyContinue | Where-Object {
      $_.Path -in @((Join-Path $binaryRoot "amps.exe"), (Join-Path $binaryRoot "amps-ui.exe"))
   })
   $task = Get-ScheduledTask -TaskName AMPS -ErrorAction SilentlyContinue
   $resumeArray = $processes.Count -gt 0 -or ($task -and $task.State -eq "Running")
   if ($resumeArray -and -not $task) { throw "Cannot safely restore AMPS: its scheduled task is missing." }
   ...
   try {
      if ($resumeArray) {
         Stop-ScheduledTask -TaskName AMPS
      ...
      try {
         ...
         if ($resumeArray) { Start-ScheduledTask -TaskName AMPS }
      }
   }
   ...
   if (-not $ready) { throw "VAC restart completed but seven active cable pairs did not appear. Existing AMPS was restarted; no reboot was attempted." }
   if ($LogPath) { "VAC capacity is $desiredCount; seven active playback/recording pairs verified. Existing AMPS resumed." | Set-Content -LiteralPath $LogPath -Encoding UTF8 }
   Write-Host "VAC capacity reconciled; existing AMPS resumed."
   exit 0
}

if (-not ("AMPS.EndpointPolicy" -as [type])) {
...
$restoreNames = @{}
if ($RestoreNamesPath) {
   $records = Get-Content -LiteralPath $RestoreNamesPath -Raw | ConvertFrom-Json
   if ($records.Count -ne 14) { throw 'Endpoint rollback requires all fourteen original endpoints.' }
   foreach ($record in $records) {
      if ($restoreNames.ContainsKey($record.ItemId) -or $record.Name -notmatch '^(?:AudioArray|AMPS) ' -and $record.Name -notmatch '^Line [1-7]$') {
         throw 'Invalid or duplicate endpoint rollback record.'
      }
      $restoreNames[$record.ItemId] = $record.Name
   }
}
...
foreach ($direction in @("Render", "Capture")) {
   ...
   foreach ($endpoint in Get-ChildItem -LiteralPath $directionRoot -ErrorAction SilentlyContinue) {
      ...
      $endpointPrefix = if ($direction -eq 'Render') { '{0.0.0.00000000}' } else { '{0.0.1.00000000}' }
      $endpointId = "$endpointPrefix.$($endpoint.PSChildName)"
      if ($RestoreNamesPath) {
         if (-not $restoreNames.ContainsKey($endpointId)) { throw "Refusing to restore an endpoint with a changed ID: $endpointId" }
         $desiredName = $restoreNames[$endpointId]
      }
      if ($properties.$friendlyNameProperty -ne $desiredName) {
         ...
         $endpointId = "$endpointPrefix.$($endpoint.PSChildName)"
         [AMPS.EndpointPolicy]::SetDeviceDescription($endpointId, $desiredName)
...
if ($missing) {
   throw "Could not identify every AMPS VAC endpoint: $($missing -join ', ')."
}

if ($changed) {
   Start-Sleep -Seconds 2
   Write-Host "Windows Audio published the new AMPS endpoint names."
} else {
   Write-Host "AMPS endpoint names are current."
}

if ($LogPath) {
   "AMPS endpoint naming completed successfully." |
