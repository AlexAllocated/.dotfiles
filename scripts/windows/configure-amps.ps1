...
. (Join-Path $PSScriptRoot 'amps-launch-context.ps1')
if (Test-AmpsPackagedHost) {
   Invoke-AmpsUnpackaged -ScriptPath $PSCommandPath -Parameters $PSBoundParameters
   exit 0
}
...
trap {
	...
	if ($restoreOnFailure -and $deploymentBackup) {
		try {
			Restore-AmpsDeployment -Backup $deploymentBackup
...
$applicationRoot = Join-Path $env:LOCALAPPDATA "AMPS"
$legacyRoot = Join-Path $env:LOCALAPPDATA "AudioArray"
$legacyConfigRoot = Join-Path $env:APPDATA "AudioArray"
$buildRoot = Join-Path $applicationRoot "build"
$binaryRoot = Join-Path $applicationRoot "bin"
$binaryPath = Join-Path $binaryRoot "amps.exe"
$uiBinaryPath = Join-Path $binaryRoot "amps-ui.exe"
$configRoot = Join-Path $env:APPDATA "AMPS"
$configPath = Join-Path $configRoot "config.toml"
$logRoot = Join-Path $applicationRoot "logs"
$logPath = Join-Path $logRoot "amps.log"
$launcherPath = Join-Path $applicationRoot "run-amps.vbs"
$stampPath = Join-Path $applicationRoot "source.sha256"
$configBasePath = Join-Path $applicationRoot 'config-base.toml'
$endpointConfiguratorSource = Join-Path $PSScriptRoot "configure-amps-endpoints.ps1"
$endpointConfiguratorPath = Join-Path $applicationRoot "configure-amps-endpoints.ps1"
$endpointConfiguratorLogPath = Join-Path $logRoot "configure-amps-endpoints.log"
$taskName = "AMPS"
...
$uiShortcutPath = Join-Path $shortcutRoot "AMPS.lnk"
$webStoragePath = Join-Path $env:LOCALAPPDATA 'ai.hivetech.audioarray\EBWebView\Default\Local Storage\leveldb'
...
function Get-SourceHash {
   ...
   $hashText = foreach ($relativePath in $relativePaths | Sort-Object) {
      ...
      if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
         throw "AMPS source file is missing: $path"
      }
...
function Assert-AMPSApplicationPolicy {
   ...
   throw @"
Smart App Control is enforcing and the locally-built AMPS interface is
...
Defender, SmartScreen, Secure Boot, and TPM protections enabled, or AMPS
...
function Test-AMPSRunning {
   $interface = Get-Process amps-ui -ErrorAction SilentlyContinue |
      Where-Object { $_.Path -eq $uiBinaryPath } |
      Select-Object -First 1
   $engine = Get-Process amps -ErrorAction SilentlyContinue |
      Where-Object { $_.Path -eq $binaryPath } |
      Select-Object -First 1
   return [bool]($interface -and $engine)
}

function Test-AMPSTaskCurrent {
...
function Install-AMPSShortcut {
   ...
   try {
      ...
      if (
         ...
         $shortcut.Description -eq "AMPS Intrepid operations console"
      ) {
         return $false
      }
      ...
      $shortcut.Description = "AMPS Intrepid operations console"
      $shortcut.Save()
      return $true
   } finally {
...
function Get-VacEndpoints {
   ...
   foreach ($direction in @("Render", "Capture")) {
      ...
      foreach ($endpoint in Get-ChildItem -LiteralPath $directionRoot -ErrorAction SilentlyContinue) {
         ...
         if ($currentName -notmatch '^Line [1-7]$' -and $currentName -notmatch '^(?:AMPS|AudioArray) (Game|Comms(?: In| Send| Audio| Mic)?|Music|Media|AI (?:Audio|Mic)|ChatGPT(?: Out| In)?|Discord Send|Clean Mic)$') {
            continue
         }
...
function Assert-AMPSEndpointNames {
   $expected = @{
      1 = "AMPS Game"
      2 = "AMPS Comms Audio"
      3 = "AMPS Media"
   ...
   if (-not $incorrect) {
      Write-Host "AMPS endpoint names are current."
      return $false
   }
   ...
   if ($LASTEXITCODE -ne 0) {
      ...
      throw "AMPS endpoint naming failed with exit code $LASTEXITCODE.`n$details"
   }
   ...
   if ($remaining) {
      throw "Windows did not retain every AMPS endpoint name."
   }
   Write-Host "Reconciled AMPS endpoint names."
   return $true
}

function Set-AMPSSpatialFormats {
   ...
   $exportPath = Join-Path $env:TEMP "amps-spatial-$([guid]::NewGuid().ToString('N')).csv"
...
"@
      ...
      if ($changed) {
         Write-Host "Reconciled AMPS spatial formats."
      } else {
         Write-Host "AMPS spatial formats are current."
      }
   } finally {
      Remove-Item -LiteralPath $exportPath -Force -ErrorAction SilentlyContinue
   }
...
function Set-AMPSUnityGain {
   $soundVolumeView = Get-SoundVolumeViewPath
	$preserveMaster = Test-AMPSRunning
   $exportPath = Join-Path $env:TEMP "amps-levels-$([guid]::NewGuid().ToString('N')).csv"

   function Get-AMPSLevelItems {
      Remove-Item -LiteralPath $exportPath -Force -ErrorAction SilentlyContinue
      Start-Process `
         -FilePath $soundVolumeView `
         -ArgumentList @("/scomma", "`"$exportPath`"") `
      ...
      return @(Import-Csv -LiteralPath $exportPath | Where-Object {
         $_."Device Name" -eq "Virtual Audio Cable" -and (
            ($_.Type -eq "Device" -and $_.Name -match '^(?:AMPS|AudioArray) (Game|Comms(?: In| Send| Audio| Mic)?|Music|Media|AI (?:Audio|Mic)|ChatGPT(?: Out| In)?|Discord Send|Clean Mic)$') -or
            $_.Type -eq "Subunit"
         )
      })
   }
   ...
   try {
      $items = @(Get-AMPSLevelItems)
      if ($items.Count -lt 28) {
         throw "Expected at least 28 AMPS endpoint and pin gain stages; found $($items.Count)."
      }
      $itemsToUpdate = @($items | Where-Object { -not (Test-UnityGain -Item $_) })
      if ($itemsToUpdate.Count -eq 0) {
         Write-Host "All AMPS endpoint and Main pin gain stages are already at 0.00 dB."
         return $false
      }
      ...
      Start-Sleep -Seconds 1
      $remaining = @(Get-AMPSLevelItems | Where-Object { -not (Test-UnityGain -Item $_) })
      if ($remaining) {
         $descriptions = $remaining | ForEach-Object {
            "$($_.Name) [$($_.Direction)]"
         }
         throw "AMPS gain verification failed for: $($descriptions -join ', ')."
      }
      Write-Host "Reconciled $($itemsToUpdate.Count) AMPS gain stage(s) to 0.00 dB."
   } finally {
      Remove-Item -LiteralPath $exportPath -Force -ErrorAction SilentlyContinue
   }
   return $true
...
foreach ($requiredPath in @(
   ...
   if (-not (Test-Path -LiteralPath $requiredPath -PathType Leaf)) {
      throw "AMPS source is incomplete: $requiredPath"
   }
}

Assert-NativeBuildTools
if (-not (Test-Path -LiteralPath $buildRoot) -and (Test-Path -LiteralPath (Join-Path $legacyRoot 'build'))) {
   New-Item -ItemType Directory -Path $applicationRoot -Force | Out-Null
   Move-Item -LiteralPath (Join-Path $legacyRoot 'build') -Destination $buildRoot
   # Tauri embeds absolute permission-file paths in its build metadata.
   # Regenerate only that package's cached outputs after relocating the cache.
   $env:CARGO_TARGET_DIR = $buildRoot
   $clean = Start-Process -FilePath $cargoPath -ArgumentList @('clean', '--release', '-p', 'tauri', '--manifest-path', $uiManifestPath) -NoNewWindow -PassThru
   $null = $clean.Handle
   $clean.WaitForExit(); $clean.Refresh()
   if ($clean.ExitCode -ne 0) { throw 'Could not refresh relocated Tauri build metadata.' }
   Write-Host 'Reused the existing Rust build cache under AMPS; installed AudioArray binaries are untouched.'
}
...
if ($rebuilt) {
   ...
   if ($frontend -match '^\\\\(?:wsl\.localhost|wsl\$)\\([^\\]+)\\(.+)$') {
      ...
      if ($LASTEXITCODE -ne 0) { throw "AMPS frontend build failed; live binaries remain unchanged." }
   } else {
      Push-Location $frontend
      try {
         & npm.cmd ci --no-fund --no-audit
         if ($LASTEXITCODE -ne 0) { throw "AMPS frontend dependency installation failed." }
         & npm.cmd run build
         if ($LASTEXITCODE -ne 0) { throw "AMPS frontend build failed." }
      } finally { Pop-Location }
   }
   $env:CARGO_TARGET_DIR = $buildRoot
   $builtBinary = Join-Path $buildRoot "release\amps.exe"
   $builtUiBinary = Join-Path $buildRoot "release\amps-ui.exe"
   ...
   if (-not (Test-Path -LiteralPath $builtBinary -PathType Leaf)) {
      throw "AMPS build completed without producing $builtBinary."
   }
   Invoke-CargoReleaseBuild -CargoManifestPath $uiManifestPath
   if (-not (Test-Path -LiteralPath $builtUiBinary -PathType Leaf)) {
      throw "AMPS interface build completed without producing $builtUiBinary."
   }
   Assert-AMPSApplicationPolicy -InterfacePath $builtUiBinary
}

if ($BuildOnly) {
   Write-Host 'AMPS build preflight complete; no running app, endpoints, preferences, or startup entries were changed.'
   exit 0
}
...
if (Test-Path -LiteralPath $vacRegistryPath) {
   ...
   if ($LASTEXITCODE -ne 0) { throw "VAC capacity preflight failed; installed AMPS binaries and configuration were left unchanged." }
}

$desiredConfig = Get-AmpsDesiredConfig -Current $configRoot -Legacy $legacyConfigRoot -Example $exampleConfigPath -PreviousBase $configBasePath
$configChanged = -not (Test-Path -LiteralPath $configPath) -or $desiredConfig -cne [IO.File]::ReadAllText($configPath)
$needsTransaction = $rebuilt -or $configChanged -or
   [bool](Get-ScheduledTask -TaskName AudioArray -ErrorAction SilentlyContinue) -or
   [bool](@(Get-VacEndpoints | Where-Object { $_.Name -notlike 'AMPS *' }).Count)
if ($needsTransaction) {
   $deploymentBackup = Start-AmpsDeployment
   $restoreOnFailure = $true
   Initialize-AmpsDeploymentState -Backup $deploymentBackup
}
if ($rebuilt) {
   $copied = $false
   ...
   if (-not $copied) {
      throw "An AMPS executable remained locked after stopping its processes."
   }
   Set-Content -LiteralPath $stampPath -Value $sourceHash -Encoding ASCII
   Write-Host "Built and installed AMPS engine and interface in $binaryRoot."
} else {
   Write-Host "AMPS engine and interface are current."
}

$shortcutChanged = Install-AMPSShortcut
if ($shortcutChanged) {
   Write-Host "Updated the AMPS Start-menu shortcut."
} else {
   Write-Host "AMPS Start-menu shortcut is current."
}

if ($configChanged) {
   [IO.File]::WriteAllText($configPath, $desiredConfig, [Text.UTF8Encoding]::new($false))
   Write-Host "Updated AMPS config at $configPath."
} else {
   Write-Host "AMPS config is current."
}
if ($desiredConfig -cne [IO.File]::ReadAllText($exampleConfigPath)) {
   Write-Host 'Preserved customized AMPS base configuration; review config.example.toml to opt into new defaults.'
}
if (-not (Test-FileEqual -Source $exampleConfigPath -Destination $configBasePath)) {
   Copy-Item -LiteralPath $exampleConfigPath -Destination $configBasePath -Force
}
...
AMPS is built but will not auto-start until amps doctor passes.
"@
   if ($OpenVacDownload) {
...
$endpointNamesChanged = Assert-AMPSEndpointNames
...
$null = Set-AMPSSpatialFormats
$null = Set-AMPSUnityGain
$taskChanged = -not (Test-AMPSTaskCurrent)
...
if ($doctorExitCode -ne 0) {
	if ($restoreOnFailure) { throw "Replacement AMPS failed its endpoint check; restoring the previous installation." }
   Unregister-ScheduledTask -TaskName $taskName -Confirm:$false -ErrorAction SilentlyContinue
...
} elseif (-not (Test-AMPSRunning)) {
   Stop-AMPS
   Start-ScheduledTask -TaskName $taskName
   Write-Host "AMPS configuration was current; restarted its stopped supervisor and engine."
} else {
   Write-Host "AMPS configuration and processes are current; no restart was needed."
}
if ($deploymentBackup) {
	...
	do {
		...
		try {
         ...
         $verified = $status.online -and -not $status.error -and ($null -ne $status.appliedRevision) -and ($status.revision -eq $status.appliedRevision)
		} catch { $verified = $false }
	} until ($verified -or (Get-Date) -ge $deadline)
	if (-not $verified) { throw "New AMPS did not acknowledge an active graph; restoring the previous installation." }
	Complete-AmpsDeployment -Backup $deploymentBackup
	$restoreOnFailure = $false
...
Write-Host "AMPS's tray supervisor and audio engine are running and will start at every interactive logon. Logs: $logPath"
