...
declare global {
	interface Window {
		...
		__AUDIOARRAY_TEST__?: { invoke: (cmd: string, args?: any) => Promise<any> };
	}
}
const invoke = window.__TAURI__?.core.invoke ?? window.__AUDIOARRAY_TEST__?.invoke;
...
function Console() {
	...
	useEffect(() => {
		if (!invoke) {
			setError("No native AudioArray bridge. This page cannot change routing.");
			return;
		}
		let active = true,
			inflight = false;
			...
			<div className="console">
				<header className="masthead">
					...
					<div className="connection-state">
						...
						{online ? "ARRAY ONLINE" : "ARRAY OFFLINE"}
						<small>
							{snapshot?.runtime ?
