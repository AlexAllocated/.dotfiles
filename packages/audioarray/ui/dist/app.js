...
function renderSnapshot(snapshot) {
	...
	elements.filterValue.textContent = snapshot.suppressionEnabled
		? `DFN3 ${snapshot.suppressionIntensity}%`
		: "BYPASSED";
	...
	elements.mainInputPolicy.textContent = snapshot.sessionInputOverride
		...
		: snapshot.suppressionEnabled
			? "This source feeds DeepFilterNet3, then becomes AudioArray Clean Mic."
	...
	elements.graphSuppressionName.textContent = snapshot.suppressionEnabled ? "DEEPFILTERNET3" : "FILTER BYPASS";
...
function updateSuppressionReadout() {
	...
	elements.suppressionIntensityValue.textContent = `${intensity}% · ${attenuation.toFixed(0)} DB`;
}
...
async function applySuppression() {
	...
	elements.routingState.textContent = "RESTARTING CLEAN MIC";
...
function buildWaveformRoutes() {
	...
	for (const base of document.querySelectorAll("#signalGraph .signal-lines > path.signal[data-meter]")) {
...
function renderWaveform(reading) {
	...
	for (const route of routes) {
		const points = route.geometry.map((point) => {
			const samplePosition = Math.min(reading.waveform.length - 1, point.distance / WAVEFORM_PIXELS_PER_SAMPLE);
			const lowerIndex = Math.floor(samplePosition);
			const upperIndex = Math.min(reading.waveform.length - 1, lowerIndex + 1);
...
elements.suppressionIntensity.addEventListener("input", updateSuppressionReadout);
